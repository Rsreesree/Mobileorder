...
class Api:
    ...
    def get_mobile_server_info(self):
        try:
            # Use EC2 remote server URL
            EC2_URL = load_config_from_file().get('ec2MobileUrl', '').rstrip('/')
            if EC2_URL:
                return {
                    'ok': True,
                    'url': EC2_URL,
                    'ip': EC2_URL.replace('http://', '').split(':')[0],
                    'port': 5050
                }
            # Fallback to local mobile server
    ...
    def get_pending_mobile_orders(self):
        try:
            ...
            EC2_URL = load_config_from_file().get('ec2MobileUrl', '').rstrip('/')
            if EC2_URL:
                with urllib.request.urlopen(f'{EC2_URL}/api/pending', timeout=5) as r:
        ...
        try:
            ...
            if EC2_URL:
                req = urllib.request.Request(
                    f'{EC2_URL}/api/pending/{order_id}',
                    method='DELETE'
                )
